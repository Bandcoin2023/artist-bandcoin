...
- Do NOT ask questions, just pass the pins array directly to the tool.`;
  ...
  return `
You are the Bandcoin Pin Generation Agent.
TODAY: ${todayISO()}
CURRENT STEP: ${step}
...
function buildJsonPrompt(state: AgentState, toolData: ToolPassData, userMessage: string): string {
  ...
  return `
You are the Bandcoin response formatter. Output ONLY a single JSON object — no prose, no markdown, no code fences.
TODAY: ${t}
IN_100_YEARS: ${y}
...
function buildQuickMessagePrompt(step: AgentStep, state: AgentState, userMessage: string): string {
  ...
  return `You are a friendly assistant for a pin generation app called Bandcoin.
Context: ${ctx}
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (detectIntentReset(message, currentStep)) {
        const resetResponse = await generateText({
          ...
          messages: [{
            role: "user" as const,
            content: `You are a friendly assistant for a pin creation app called Bandcoin.
