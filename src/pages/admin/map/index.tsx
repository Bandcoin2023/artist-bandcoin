...
import { MapPin, Search, X, Plus, Minus, ArrowRightFromLine, ArrowLeftFromLine } from "lucide-react"
        ...
        <AdminLayout>
            <APIProvider apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAP_API_KEY!}>
                ...
                >
                    ...
                    {selectedCreator && (
                        <MyPins onOpen={onOpen} setIsAutoCollect={setIsAutoCollect} creatorId={selectedCreator?.id} />
                    )}
                </Map>
                ...
                {creator.data && (
                    <motion.div
                        className="absolute top-16 left-6 z-10 w-64"
...
function ManualPinButton({ handleClick }: { handleClick: () => void }) {
    return (
        <motion.div
            ...
            whileTap={{ scale: 0.95 }}
        >
            <Button onClick={handleClick} className="flex items-center gap-2 shadow-md">
      ...
      `}</style>
            <motion.div
                className="absolute z-10 flex items-center gap-2
                 bottom-4 left-2 md:bottom-auto md:left-auto md:top-4 md:right-2"
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
            ...
            >
                ...
                >
                    <Button
                        variant="destructive"
                        size="icon"
                        className="h-10 w-10 rounded-full shadow-sm shadow-foreground touch-manipulation select-none"
                        onClick={onZoomOut}
                        aria-label="Zoom out"
                    >
