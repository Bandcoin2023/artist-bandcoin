...
type RedeemStatus =
  ...
  | "not_found"
  | "wrong_location";
type PinType = "EVENT" | "BOUNTY" | "EXPERIENCE" | "LAUNCH" | "OTHER";
...
function TypeBadge({ type }: { type: PinType }) {
  const map: Record<
    PinType,
  ...
  > = {
    ...
    EXPERIENCE: {
      ...
      cls: "[background-color:hsl(var(--success)/0.12)] [color:hsl(var(--success))] [border-color:hsl(var(--success)/0.3)]",
    },
    LAUNCH: {
      label: "Launch",
      icon: <Eye className="h-2.5 w-2.5" />,
      cls: "[background-color:hsl(var(--accent)/0.12)] [color:hsl(var(--accent))] [border-color:hsl(var(--accent)/0.3)]",
    },
    OTHER: {
      label: "Other",
      icon: <Hash className="h-2.5 w-2.5" />,
      cls: "[background-color:hsl(var(--muted)/0.12)] [color:hsl(var(--muted-foreground))] [border-color:hsl(var(--border)/0.3)]",
    },
  };
  const cfg = map[type] ?? map.OTHER;
  return (
    <span
      className={`flex items-center gap-1 rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${cfg.cls}`}
    >
      {cfg.icon}
      {cfg.label}
    </span>
    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        ...
        {search && (
          <button
            onClick={() => onSearch("")}
            className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-0.5 transition-colors [color:hsl(var(--muted-foreground))] hover:[background-color:hsl(var(--muted)/0.4)]"
          >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {(["EVENT", "BOUNTY", "EXPERIENCE"] as PinType[]).map((t) => (
            <button
              key={t}
                                  ...
                                  type === t
                                    ? t === "EVENT"
                                      ...
                                      : t === "BOUNTY"
                                        ...
                                        : "border-transparent [background-color:hsl(var(--success)/0.15)] [color:hsl(var(--success))]"
...
function InfiniteScrollSentinel({
  ...
  return (
    <div ref={ref} className="flex justify-center py-4">
      {isLoading && (
        <Loader2 className="h-5 w-5 animate-spin [color:hsl(var(--muted-foreground))]" />
      )}
    </div>
  );
}
          ...
          >
            <div className="w-full max-w-xl overflow-hidden rounded-3xl border shadow-2xl [background-color:hsl(var(--card))] [border-color:hsl(var(--border)/0.2)]">
              <div className="flex items-center justify-between border-b px-6 py-4 [border-color:hsl(var(--border)/0.1)]">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest [color:hsl(var(--muted-foreground))]">
                    Confirm Redemption
                  </p>
                  <h2 className="text-base font-black [color:hsl(var(--foreground))]">
                    Review before redeeming
                  </h2>
                </div>
                <button
                  onClick={onClose}
                  className="rounded-lg p-1.5 [color:hsl(var(--muted-foreground))] hover:[background-color:hsl(var(--muted)/0.3)]"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="space-y-4 p-6">
                ...
                <div className="rounded-2xl border p-3 [background-color:hsl(var(--muted)/0.1)] [border-color:hsl(var(--border)/0.15)]">
                  <p className="mb-2 text-[10px] font-bold uppercase tracking-widest [color:hsl(var(--muted-foreground))]">
                    Location Group
                  </p>
                  <div className="flex items-start gap-3">
                    {consumer.location.image ? (
                      <div className="relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-xl">
                        <img
                          src={consumer.location.image}
                          alt={consumer.location.title}
                ...
                <div className="rounded-2xl border p-3 [background-color:hsl(var(--muted)/0.1)] [border-color:hsl(var(--border)/0.15)]">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <LocationAddressDisplay
                      latitude={consumer.locationData.latitude}
                      longitude={consumer.locationData.longitude}
                    />
                    <span className="font-mono text-xs [color:hsl(var(--muted-foreground))]">
                      {consumer.locationData.latitude.toFixed(5)},{" "}
                      {consumer.locationData.longitude.toFixed(5)}
                    </span>
                  </div>
                </div>
                {/* Code */}
                <div className="flex items-center justify-between rounded-xl border px-4 py-2.5 [background-color:hsl(var(--muted)/0.1)] [border-color:hsl(var(--border)/0.15)]">
                  <span className="flex items-center gap-1.5 text-xs [color:hsl(var(--muted-foreground))]">
                    <Hash className="h-3 w-3" /> Code
                  </span>
                  <span className="font-mono text-sm font-black tracking-widest [color:hsl(var(--foreground))]">
                    {consumer.redeemCode}
                  </span>
                </div>
              </div>
              <div className="flex gap-3 border-t px-6 py-4 [border-color:hsl(var(--border)/0.1)]">
                <Button
                  variant="ghost"
                  onClick={onClose}
                  className="h-11 flex-1 rounded-2xl text-sm font-semibold [color:hsl(var(--muted-foreground))] hover:[background-color:hsl(var(--muted)/0.3)]"
                >
                  Cancel
                </Button>
                <Button
                  onClick={onConfirm}
                  disabled={isLoading}
                  className="h-11 flex-1 rounded-2xl text-sm font-bold [background-color:hsl(var(--primary))] [color:hsl(var(--primary-foreground))] hover:[background-color:hsl(var(--primary)/0.9)]"
                >
                  {isLoading ? (
                    <>
                    ...
                    <>
                      <ShieldCheck className="mr-2 h-4 w-4" /> Confirm Redeem
                    </>
                  )}
                </Button>
              </div>
...
function LocationRow({ location, locationGroup }: LocationRowProps) {
  ...
  const [previewData, setPreviewData] =
    useState<PreRedeemModalProps["consumer"]>(null);
  ...
  const lookupMutation = api.maps.pin.lookupRedeemCode.useMutation({
    onSuccess: (data) => {
      ...
      if (data.status === "wrong_location") {
        const locationName =
          data.actualLocation?.groupTitle ?? "another location";
        setLookupError(
          `This code belongs to ${locationName}, not this location.`,
        );
        return;
      }
  ...
  const redeemMutation = api.maps.pin.redeemByCode.useMutation({
    onSuccess: (data) => {
      setResultStatus(data.status as RedeemStatus);
      setShowModal(false);
      setCode("");
    },
    onError: () => {
  ...
  const progressPct =
    totalConsumers > 0 ? Math.round((redeemedCount / limit) * 100) : 0;

  return (
    <>
      {/* Ticket card */}
      <div className="[border-color:hsl(var(--border)] relative flex overflow-hidden rounded-2xl border [background-color:hsl(var(--card))]">
        ...
        <div className="pointer-events-none absolute inset-y-0 left-[55%] z-10 flex flex-col items-center">
          ...
          {/* Dashed line */}
          <div className="[border-color:hsl(var(--border)] flex-1 border-l-[1.5px] border-dashed" />
          {/* Bottom notch */}
          <div className="[border-color:hsl(var(--border)] h-3 w-3 -translate-x-[5px] translate-y-1.5 rounded-full border [background-color:hsl(var(--background))]" />
        </div>
        ...
        <div className="flex w-[55%] flex-col gap-3 p-4 pr-6">
          {/* Address */}
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-widest [color:hsl(var(--muted-foreground))]">
              Pin location
            </p>
            <LocationAddressDisplay
              latitude={location.latitude}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-xl border px-3 py-2 [background-color:hsl(var(--card)/0.6)] [border-color:hsl(var(--border)/0.15)]">
              <p className="text-base font-black [color:hsl(var(--success))]">
                {redeemedCount}
              </p>
              <p className="text-[10px] uppercase tracking-wide [color:hsl(var(--muted-foreground))]">
                Redeemed
              </p>
            </div>

            <div className="rounded-xl border px-3 py-2 [background-color:hsl(var(--card)/0.6)] [border-color:hsl(var(--border)/0.15)]">
              <p className="text-base font-black [color:hsl(var(--foreground))]">
                {limit}
              </p>
              <p className="text-[10px] uppercase tracking-wide [color:hsl(var(--muted-foreground))]">
                Limit
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              ...
              <p className="text-[10px] font-black [color:hsl(var(--foreground)/0.6)]">
                {progressPct}%
              </p>
            </div>
        ...
        <div className="flex w-[45%] flex-col justify-center gap-3 p-4 pl-6">
          <AnimatePresence mode="wait">
            {resultStatus ? (
              ...
              >
                <div
                  className={`flex items-center gap-2 rounded-xl border px-3 py-2.5 ${STATUS_UI[resultStatus].cls}`}
                >
                  {STATUS_UI[resultStatus].icon}
                  <span className="text-xs font-semibold">
                    {STATUS_UI[resultStatus].label}
                  </span>
                </div>
                <button
                  onClick={handleReset}
              ...
              >
                <p className="text-center text-[10px] font-bold uppercase tracking-widest [color:hsl(var(--muted-foreground))]">
                  Redeem code
                </p>
                ...
                <div className="flex justify-center gap-1.5">
                  {Array.from({ length: 6 }).map((_, i) => (
                    <div
                      key={i}
                      className={`h-1.5 w-1.5 rounded-full transition-all duration-100 ${
                        i < code.length
                          ? code.length === 6
...
function GroupCard({ group }: GroupCardProps) {
  ...
  return (
    <motion.div
      layout
      className="overflow-hidden rounded-3xl border [background-color:hsl(var(--card)/0.5)] [border-color:hsl(var(--border)/0.2)]"
    >
      <button
      ...
      >
          ...
          {group.image ? (
            ...
            <div className="flex h-full w-full items-center justify-center">
              {group.type === "EVENT" ? (
                ...
                <Hash className="h-6 w-6 [color:hsl(var(--muted-foreground)/0.4)]" />
              ) : (
                <MapPin className="h-6 w-6 [color:hsl(var(--muted-foreground)/0.4)]" />
              )}
            </div>
          )}
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1.5 flex flex-wrap items-center gap-2">
            <span className="text-sm font-black [color:hsl(var(--foreground))]">
              {group.title}
            </span>
      ...
      {allGroups.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-3xl border [background-color:hsl(var(--card)/0.5)] [border-color:hsl(var(--border)/0.3)]">
            {typeFilter === "EVENT" ? (
              <PartyPopper className="h-7 w-7 [color:hsl(var(--muted-foreground))]" />
            ) : typeFilter === "BOUNTY" ? (
              <Hash className="h-7 w-7 [color:hsl(var(--muted-foreground))]" />
            ) : (
              <MapPin className="h-7 w-7 [color:hsl(var(--muted-foreground))]" />
            )}
          </div>
          <div>
            <p className="text-sm font-bold [color:hsl(var(--foreground))]">
            ...
            <p className="mt-1 text-xs [color:hsl(var(--muted-foreground))]">
              {search
                ...
                : "Create a location group to get started."}
            </p>
          </div>
          {search && (
            <button
              onClick={() => setSearch("")}
              className="text-xs underline [color:hsl(var(--primary))]"
            >
              Clear search
            </button>
          )}
        ...
        <div className="space-y-3">
          ...
          {!hasNextPage && allGroups.length > 0 && (
            <p className="py-4 text-center text-xs [color:hsl(var(--muted-foreground)/0.5)]">
              All {allGroups.length} group{allGroups.length !== 1 ? "s" : ""}{" "}
              loaded
            </p>
          )}
        </div>
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 z-40 backdrop-blur-sm [background-color:hsl(0_0%_0%/0.5)]"
          />
          <motion.div
          ...
          >
            <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl border shadow-2xl [background-color:hsl(var(--card))] [border-color:hsl(var(--border)/0.2)]">
              <div className="sticky top-0 flex items-center justify-between border-b px-6 py-4 [background-color:hsl(var(--card))] [border-color:hsl(var(--border)/0.1)]">
                <h2 className="text-sm font-black [color:hsl(var(--foreground))]">
                  Redemption Details
                </h2>
                <button
                  onClick={onClose}
              ...
              <div className="space-y-4 p-6">
                <div>
                  ...
                  <div className="rounded-xl border px-4 py-2.5 font-mono text-sm font-black tracking-widest [background-color:hsl(var(--muted)/0.15)] [border-color:hsl(var(--border)/0.2)] [color:hsl(var(--foreground))]">
                    {item.redeemCode}
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5 rounded-2xl border px-4 py-3 [background-color:hsl(var(--muted)/0.1)] [border-color:hsl(var(--border)/0.15)]">
                    ...
                    />
                    <p className="font-mono text-xs [color:hsl(var(--muted-foreground))]">
                      {item.locationData.latitude.toFixed(5)},{" "}
                      {item.locationData.longitude.toFixed(5)}
                    </p>
                  </div>
                </div>
                <div className="space-y-2">
                  {item.claimedAt && (
                    <div className="flex items-center justify-between rounded-xl border px-3 py-2 text-xs [background-color:hsl(var(--muted)/0.1)] [border-color:hsl(var(--border)/0.15)]">
                      <span className="flex items-center gap-1.5 [color:hsl(var(--muted-foreground))]">
                        <MapPin className="h-3 w-3" /> Claimed
                      </span>
                      <span className="font-semibold [color:hsl(var(--foreground))]">
                  ...
                  {item.redeemedAt && (
                    <div className="flex items-center justify-between rounded-xl border px-3 py-2 text-xs [background-color:hsl(var(--success)/0.08)] [border-color:hsl(var(--success)/0.2)]">
                      <span className="flex items-center gap-1.5 [color:hsl(var(--success))]">
...
function HistoryTab() {
  ...
  const grouped = allItems.reduce<
    Record<string, { group: LocationGroup; items: HistoryItem[] }>
  >((acc, item) => {
        ...
        {allItems.length === 0 ? (
          ...
          <div className="space-y-4">
            {Object.values(grouped).map(({ group, items }) => (
              <div
                key={group.id}
                className="overflow-hidden rounded-3xl border [background-color:hsl(var(--card)/0.4)] [border-color:hsl(var(--border)/0.2)]"
              >
                {/* Group header */}
                <div className="flex items-center gap-3 border-b px-5 py-4 [border-color:hsl(var(--border)/0.1)]">
                  {group.image ? (
                    ...
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl [background-color:hsl(var(--primary)/0.1)]">
                      {group.type === "EVENT" ? (
                        <PartyPopper className="h-4 w-4 [color:hsl(var(--warning))]" />
                      ) : group.type === "BOUNTY" ? (
                        <Hash className="h-4 w-4 [color:hsl(var(--primary))]" />
                      ) : (
                        <MapPin className="h-4 w-4 [color:hsl(var(--success))]" />
                      )}
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-black [color:hsl(var(--foreground))]">
                        {group.title}
                      </p>
                      <TypeBadge type={group.type} />
                    </div>
                    <p className="text-xs [color:hsl(var(--muted-foreground))]">
                      {items.length} redemption{items.length !== 1 ? "s" : ""}
                    </p>
                  </div>
                </div>
                {/* Rows */}
                  ...
                  {items.map((item, i) => (
                    ...
                    >
                      ...
                      <div className="flex flex-shrink-0 flex-col items-end gap-1">
                        <span className="font-mono text-xs font-black tracking-widest [color:hsl(var(--foreground)/0.7)]">
                          {item.redeemCode}
                        </span>
            ...
            {!hasNextPage && allItems.length > 0 && (
              <p className="py-4 text-center text-xs [color:hsl(var(--muted-foreground)/0.5)]">
                All {allItems.length} redemption
  ...
  return (
    <div className="min-h-screen [background-color:hsl(var(--card)/0.5)]">
      ...
      <div className="mx-auto max-w-3xl px-5 pb-16">
        {/* Tabs */}
        <motion.div
          className="relative mx-auto my-6 inline-flex w-fit items-center gap-0.5 overflow-hidden rounded-[0.9rem] border border-black/15 p-[0.3rem] shadow-[0_8px_24px_rgba(0,0,0,0.05)]"
          ...
          <div className="relative z-10 inline-flex items-center gap-0.5">
            {(["redeem", "history"] as const).map((t) => (
              <button
                ...
                onClick={() => setTab(t)}
                className={cn(
                  "inline-flex items-center justify-center gap-1.5 rounded-[0.7rem] border border-transparent px-4 py-1.5 text-sm font-normal transition-all duration-200",
                  tab === t
                    ...
                    : "bg-transparent text-black/65 hover:bg-white/35 hover:text-black",
