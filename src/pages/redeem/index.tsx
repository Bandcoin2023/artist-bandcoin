...
function TypeBadge({ type }: { type: PinType }) {
    const map: Record<PinType, { label: string; icon: React.ReactNode; cls: string }> = {
        EVENT: {
            label: "Event",
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1.5 flex-wrap">
                    {(["EVENT", "BOUNTY", "EXPERIENCE"] as PinType[]).map((t) => (
                        ...
                        >
                            {t === "EVENT" ? <PartyPopper className="h-3 w-3" /> : t === "BOUNTY" ? <Hash className="h-3 w-3" /> : <MapPin className="h-3 w-3" />}
                            {t === "EVENT" ? "Events" : t === "BOUNTY" ? "Bounties" : "Experiences"}
                        </button>
                    ))}
                </div>
                {total !== undefined && (
            ...
            {allGroups.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
                    <div className="flex h-16 w-16 items-center justify-center rounded-3xl border [background-color:hsl(var(--card)/0.5)] [border-color:hsl(var(--border)/0.3)]">
                        {typeFilter === "EVENT" ? <PartyPopper className="h-7 w-7 [color:hsl(var(--muted-foreground))]" /> : typeFilter === "BOUNTY" ? <Hash className="h-7 w-7 [color:hsl(var(--muted-foreground))]" /> : <MapPin className="h-7 w-7 [color:hsl(var(--muted-foreground))]" />}
                    </div>
                    <div>
                        ...
                        <p className="mt-1 text-xs [color:hsl(var(--muted-foreground))]">
                            {search ? "Try a different search term." : "Create a location group to get started."}
                        </p>
                    </div>
                    {search && (
