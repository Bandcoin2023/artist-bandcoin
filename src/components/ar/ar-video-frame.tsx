...
<html>
  ...
  function tryPlay() {
    video.muted = true; // muted is required for autoplay in all modern browsers
    return video.play().then(() => {
      playBtn.classList.add('hidden');
    }).catch(() => {
      playBtn.classList.remove('hidden'); // ← was broken: used style.display instead of classList
    });
  }

  // Try once the A-Frame scene finishes loading (more reliable than setTimeout)
  document.querySelector('a-scene').addEventListener('loaded', () => tryPlay());
  // Also retry when video is ready to play
  video.addEventListener('canplay', () => { if (video.paused) tryPlay(); });

  // On explicit user tap — unmute and play
