...
export function MapboxDrawing({ onSelectionChange, mapElement, onClose, map }: MapboxDrawingProps) {
    ...
    // Use Mapbox to convert canvas coordinates to lat/lng
    const canvasCoordsToLatlng = useCallback((x: number, y: number): { lat: number; lng: number } | null => {
        ...
        try {
            // Use Mapbox's unproject to convert canvas coordinates to lng/lat
            const lngLat = map.unproject([x, rect.height - y]);
