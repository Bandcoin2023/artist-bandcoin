...
import { Check, Play, X, Download, Maximize2, Video, Trash2, Sparkles, ImageIcon, Clock } from "lucide-react"
...
export function ContentGrid() {
  const { generatedItems, toggleItemSelection, isGenerating, mediaType } = useGenerationStore()
...
function ContentCard({
  ...
  return (
    <div
      className={cn(
        "group relative aspect-square rounded-xl overflow-hidden border-2 transition-all cursor-pointer bg-secondary",
        item.selected
    ...
    >
      ...
      <div className="absolute bottom-2 left-2 right-12">
        <div className="bg-black/70 backdrop-blur-sm rounded-lg px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <p className="text-xs text-white truncate">{item.model}</p>
        </div>
      </div>
    </div>
  )
